...
let makeCircle = (x, y, resolution, restitution, radius) => {
    let o = new Obstacle(x, y, PART_OBSTACLE, makeCircleWalls(radius, resolution, restitution));
    return o;
};
...
BUILDERS[B_FLIPPER_LEFT] = (section, [x, y, restAngle, upAngle, isFlipped]) => {
    section.parts.push(new Paddle(x, y, isFlipped ? CONTROL_RIGHT : CONTROL_LEFT, restAngle || (isFlipped ? RIGHT_REST_ANGLE : LEFT_REST_ANGLE), upAngle || (isFlipped ? RIGHT_UP : LEFT_UP)));
};
...
BUILDERS[B_CIRCLE] = (section, [x, y, resolution, restitution, radius]) => {
    let circle = makeCircle(x, y, resolution, restitution, radius);
